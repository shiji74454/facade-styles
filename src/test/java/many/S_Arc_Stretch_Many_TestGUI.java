...
import unit.S_Arc_Stretch;
import wblut.geom.WB_Point;
import wblut.geom.WB_Polygon;
import wblut.processing.WB_Render3D;

public class S_Arc_Stretch_Many_TestGUI extends PApplet {
    ...
    int array_y_num = 2;
    int array_z_num = 7;
    S_Arc_Stretch[][] examples = new S_Arc_Stretch[array_y_num][array_z_num];
    Camera camera;
    ...
    public static void main(String[] args) {
        PApplet.main(S_Arc_Stretch_Many_TestGUI.class.getName());
    }
    ...
    public void setup() {
        ...
        for (int i = 0; i < array_y_num; i++) {
            for (int j = 0; j < array_z_num; j++) {
                examples[i][j] = new S_Arc_Stretch(pts);
            }
        }
        ...
        for (int i = 0; i < array_y_num; i++) {
            for (int j = 0; j < array_z_num; j++) {
                panel.updatePanel(examples[i][j], "S_Arc_Stretch");
            }
        }
